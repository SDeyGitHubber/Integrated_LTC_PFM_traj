...
1. Liquid Time-Constant Networks (LTC) with multi-step ODE integration
...
replacing LSTM encoder/decoder with LTC cells (original paper implementation).

================================================================================
KEY DIFFERENCE FROM CFC VERSION
================================================================================

This model uses the ORIGINAL LTC implementation (ltc_cell.py) which:
- Follows the AAAI 2021 paper exactly (Algorithm 1)
- Uses multi-step ODE unfolds (Euler integration)
- Has sensory/recurrent layers with reversal potentials
- More accurate but slower than CfC closed-form solution

For faster production deployment, use cfc_pfm_hybrid_model.py instead.
...
Step 2: h_enc ← LTC_Encoder(embedded_history)  [Social context encoding]
Step 3: coeffs ← CoefficientProjector(h_enc)   [Dynamic PFM coefficients]

For t = 1 to T_pred (Autoregressive Decoding Loop):
    Step 4:  decoded_pred ← LTC_Decoder_step(input_t, hx)    [Raw neural prediction]
...
# Import our LTC implementations (original paper version)
from utils.ltc_cell import LTCCell, LTC
...
class LTC_PFM_HybridModel(nn.Module):
    ...
    Combines LTC encoder-decoder (original paper implementation) with 
    Physics-Informed Potential Fields.
    ...
    2. LTC Encoder:
       - Process embedded history with multi-step ODE integration
    ...
    4. Autoregressive Decoding Loop (for t = 1..T):
       a. LTC Decoder step: predict raw displacement
    ...
    Args:
        ...
        hidden_size: LTC hidden state dimension
        ...
        ode_unfolds: Number of Euler integration steps per forward pass
    ...
    def __init__(
        ...
        # =================================================================
        # LTC ENCODER (Original Paper Implementation)
        # =================================================================
        # Uses multi-step ODE integration with Euler method
        # 
        # Mathematical Dynamics:
        # dx/dt = -[1/τ + f(x,I,θ)] x + f(x,I,θ) A
        # 
        # Integration:
        # For i = 1 to ode_unfolds:
        #     x ← x + (Δt/ode_unfolds) · dx/dt
        # 
        ...
        # =================================================================
        # LTC DECODER (Original Paper Implementation)
        # =================================================================
    ...
    def forward_encoder(self, x: torch.Tensor) -> tuple:
        """
        LTC encoder forward pass (for checkpointing).
    ...
    def forward_decoder_step(
        ...
        """
        Single LTC decoder step (for checkpointing).
    ...
    def forward(
        ...
        # h_enc is a tensor [B*A*ent, hidden_size]
